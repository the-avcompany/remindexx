...
export const ContentManager: React.FC<ContentManagerProps> = ({ userId, subjects, contents, reviews, onDataChange }) => {
  ...
  const handleAddSubject = async () => {
    ...
    await StorageService.addSubject(userId, newSubjectName);
    setNewSubjectName('');
  ...
  const handleAddContent = async () => {
    ...
    await StorageService.addContentWithReviews(userId, newContentSubject, newContentTopic, newContentDate, Number(newContentDifficulty));
    setNewContentTopic('');
  ...
  const handleRetentionAdjust = async (contentId: string, type: RetentionEventType) => {
    await StorageService.adjustSchedule(userId, contentId, type);
    onDataChange();
  };

  const handleMarkReviewDone = async (reviewId: string) => {
    await StorageService.updateReviewStatus(reviewId, ReviewStatus.COMPLETED);
    onDataChange();
  }
