...
import { Review, StudyContent, Subject, ReviewStatus, Difficulty, DayException, NavigationParams } from '../types';
...
export const CalendarView: React.FC<CalendarViewProps> = ({ reviews, contents, subjects, navParams, onDataChange }) => {
  ...
  const [exceptions, setExceptions] = useState<DayException[]>(StorageService.getExceptions(reviews[0]?.userId || ''));
  ...
  const handleMarkHeavy = () => {
    if (!selectedDateStr || !reviews.length) return;
    const userId = reviews[0].userId;
    const existing = exceptions.find(e => e.date === selectedDateStr);
    if (existing) return;
    const exception: DayException = { id: generateId(), userId, date: selectedDateStr, type: 'heavy', capacityMultiplier: 0.5 };
  ...
  const handleRedistribute = () => {
    ...
    if(confirm("Redistribuir a carga deste dia para os próximos?")) {
        PlannerService.rebalanceSchedule(reviews[0].userId);
        onDataChange();
    }
  };

  const handleAdjustMemory = (review: Review, type: 'remembered' | 'forgot') => {
      StorageService.adjustSchedule(review.userId, review.contentId, type, review.id);
      onDataChange();
  };
              ...
              <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-8">
                          ...
                          {history.map(r => {
                              ...
                              return (
                                  <div key={r.id} className={`relative ${isCurrent ? 'opacity-100 scale-100' : 'opacity-60 scale-95'} transition-all`}>
                                      <div className={`absolute -left-[31px] w-4 h-4 rounded-full border-2 ring-4 ring-white ${
                  ...
                  <div className="pt-6 border-t border-slate-100">
                      <button 
                        ...
                        className={`w-full flex items-center justify-center gap-2 p-3 rounded-lg transition-colors text-xs font-bold uppercase tracking-wide ${
