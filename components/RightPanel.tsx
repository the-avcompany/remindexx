...
import { User, Review, StudyContent, ReviewStatus, NavigationParams } from '../types';
import { formatDate, addDays, getDaysDiff, StorageService, PlannerService, ContextService, SuggestionService } from '../services';
import { BrainCircuit, Activity, Zap, Layers, ArrowRight, ShieldAlert, BarChart3, FastForward, Play, Settings } from 'lucide-react';
...
export const RightPanel: React.FC<RightPanelProps> = ({ user, reviews, contents, onDataChange, onNavigate }) => {
  ...
  const handlePushTomorrow = () => {
      ...
      if(confirm("Mover tarefas de amanhã para depois de amanhã? (Aliviar carga)")) {
          PlannerService.setTomorrowHeavy(user.id); // Simulates a heavy day to push things
          onDataChange();
      }
  };
