...
import { Review, StudyContent, Subject, ReviewStatus, ReviewFeedback, User, Difficulty, DayException, NavigationParams } from '../types';
...
export const Dashboard: React.FC<DashboardProps> = ({ user, reviews, contents, subjects, onReviewUpdate, onDataChange, onNavigate, navParams }) => {
  const [settings, setSettings] = useState(StorageService.getSettings(user.id));
  ...
  const handleAddSubject = (e?: React.FormEvent) => {
    ...
    StorageService.addSubject(user.id, newSubject.trim());
    setNewSubject('');
    onDataChange();
  };
  ...
  const handleQuickAddContent = (e: React.FormEvent) => {
    ...
    StorageService.addContentWithReviews(user.id, quickSubject, quickTopic, quickDate, Number(quickDifficulty));
    setQuickTopic('');
    setQuickSubject('');
  ...
  const handleSetTomorrowHeavy = () => {
    if(confirm('Marcar amanhã como dia pesado?')) {
      PlannerService.setTomorrowHeavy(user.id);
      onDataChange();
    }
  };

  const handlePaceChange = (mode: 'faster' | 'slower') => {
    PlannerService.setPace(user.id, mode);
    onDataChange();
  };

  const handleRedistributeOverdue = () => {
        ...
        <div className="relative z-10">
                 ...
                 <div className="flex flex-wrap gap-2 md:gap-3">
                    {subjects.map(s => (
                      <span key={s.id} className="inline-flex items-center px-3 py-1.5 md:px-4 md:py-2 rounded-full bg-white border border-slate-200 text-slate-700 text-xs md:text-sm font-medium shadow-sm">
                        {s.name}
                        <button 
                          onClick={() => { StorageService.deleteSubject(s.id); onDataChange(); }}
            ...
            {availableSuggestions.length > 0 && (
              <div className="flex flex-wrap gap-2 md:gap-3 mb-6">
                 {availableSuggestions.map(s => <button key={s} onClick={() => { StorageService.addSubject(user.id, s); onDataChange(); }} className="px-3 py-1 md:px-4 md:py-1.5 rounded-full bg-white/50 border border-primary/20 text-primary-strong text-xs md:text-sm font-medium hover:bg-white transition-colors">+ {s}</button>)}
              </div>
            )}
             
             {subjects.length > 0 && <Button onClick={() => { 
                 const newS = {...settings, setupCompleted: true}; 
                 StorageService.saveSettings(user.id, newS); 
                 setSettings(newS); 
                 onDataChange(); 
        ...
        <Card className="p-5 md:p-6 flex flex-col h-full">
            ...
            <div className="space-y-3 md:space-y-4">
                ...
                {!checklist.checkedCalendar && (
                    <Button variant="ghost" size="sm" onClick={() => {StorageService.updateChecklist(user.id, 'checkedCalendar'); onNavigate('calendar');}} className="text-sm w-full mt-2">Ir para Calendário</Button>
                )}
            </div>
        </Card>
